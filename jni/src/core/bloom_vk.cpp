...
void BloomVK::RegisterImGuiSnapshot() {
    ...
    // Fix: RegisterImGuiSnapshot is invoked again after every
    // RebuildSwapchain. Each call allocates one descriptor set out of
    // ImGui's descriptor pool; without freeing the previous one the pool
    // (maxSets = 64 in renderer_vk.cpp) is exhausted after ~64 rebuilds
    // and ImGui_ImplVulkan_AddTexture starts returning VK_NULL_HANDLE.
    if (m_PrevSceneImGuiDS != VK_NULL_HANDLE) {
        ImGui_ImplVulkan_RemoveTexture(m_PrevSceneImGuiDS);
        m_PrevSceneImGuiDS = VK_NULL_HANDLE;
    }
...
void BloomVK::Shutdown() {
    ...
    // Fix: hand the ImGui-registered descriptor back to ImGui's pool
    // BEFORE the sampled view is destroyed and BEFORE ImGui_ImplVulkan_Shutdown
    // (which drops the pool) runs. Previously the descriptor leaked on
    // every RebuildSwapchain, and the old note ("owned by imgui's pool,
    // freed at ImGui_ImplVulkan_Shutdown") only ever held on final exit.
    if (m_PrevSceneImGuiDS != VK_NULL_HANDLE) {
        ImGui_ImplVulkan_RemoveTexture(m_PrevSceneImGuiDS);
        m_PrevSceneImGuiDS = VK_NULL_HANDLE;
    }
    ...
    // Descriptor sets belonging to *our* pool are freed with the pool;
    // m_PrevSceneImGuiDS was already returned above.
    *this = BloomVK();
}
